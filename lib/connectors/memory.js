...
Memory.prototype.updateAttributes = function updateAttributes(model, id, data, options, cb) {
  ...
  if (modelData) {
    ...
    var ErrMsg = 'Could not update attributes. Object with id '
      + id + ' does not exist!';
    cb(new Error(ErrMsg));
  }
};
...
Memory.prototype.replaceById = function(model, id, data, options, cb) {
  ...
  var modelData = cachedModels && this.collection(model)[id];
  if (!modelData) {
    var ErrMsg = 'Could not replace. Object with id ' + id + ' does not exist!';
    return process.nextTick(function() { cb(new Error(ErrMsg)); });
  }
