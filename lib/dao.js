...
DataAccessObject.replaceById = function(id, data, options, cb) {
  ...
  Model.notifyObserversOf('before save', context, function(err, ctx) {
    ...
    // if ctx.instance has been changed in `before save` copy instance into data
    if (!Object.is(ctx.instance, oInst)) {
      data = {};
      for (var key in inst) {
        var val = inst[key];
        if (typeof val === 'function' || key.substring(0, 2) == '__') {
          continue; // Skip methods and internal fields
        }
        data[key] = val;
      }
    }
