...
DataAccessObject.replaceOrCreate = function replaceOrCreate(data, options, cb) {
  ...
  assert((options || cb), 'The cb must be passed');

  if (cb === undefined) {
    if (typeof options === 'function') {
      // replaceOrCreate(data,cb)
      cb = options;
      options = {};
    }
  }
  ...
  if(options.notify === false) {
    isOriginalQuery = isWhereByGivenId(Model, {}, id);
    if (typeof connector.replaceOrCreate === 'undefined' || !isOriginalQuery) {
      var ctx = {};
      ctx.query = byIdQuery(Model, id);
      ctx.options = options;
      return callNonAtomicUpdateOrCreate(ctx);
    }
    if (strict) {
    ...
    function validateAndCallConnector(err) {
      ...
      function callConnector() {
        connector.replaceOrCreate(Model.modelName, data, options, cb);
      }
    }
  } else {
    var context = {
    ...
    function doReplaceOrCreate(err, ctx) {
      ...
      isOriginalQuery = isWhereByGivenId(Model, ctx.query.where, id);
  ...
  function callNonAtomicUpdateOrCreate(ctx) {
    var opts = {notify: false};
    if (ctx.options && ctx.options.transaction) {
      opts.transaction = ctx.options.transaction;
    }
